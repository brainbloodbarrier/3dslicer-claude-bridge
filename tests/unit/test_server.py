...
class TestToolWrapperDefaults:
    ...
    Note: The registry-based _register_tool wrapper passes all arguments as
    keyword arguments (via *args/**kwargs forwarding with functools.wraps).
    Tests verify the correct values reach the feature function regardless
    of positional/keyword form.
    """
    ...
    def test_capture_screenshot_defaults(self):
        with patch("slicer_mcp.server.tools.capture_screenshot") as mock:
            ...
            assert kwargs["view_type"] == "3d"
            assert kwargs.get("scroll_position") is None
            assert kwargs.get("look_from_axis") is None
    ...
    def test_capture_screenshot_all_args(self):
        with patch("slicer_mcp.server.tools.capture_screenshot") as mock:
            ...
            mock.assert_called_once_with(
                view_type="axial", scroll_position=0.5, look_from_axis="left"
            )
    ...
    def test_measure_volume_with_segment(self):
        with patch("slicer_mcp.server.tools.measure_volume") as mock:
            ...
            mock.assert_called_once_with(node_id="seg1", segment_name="Tumor")
    ...
    def test_set_layout_with_gui_mode(self):
        with patch("slicer_mcp.server.tools.set_layout") as mock:
            ...
            mock.assert_called_once_with(layout="OneUp3D", gui_mode="viewers")
    ...
    def test_run_brain_extraction_all_args(self):
        with patch("slicer_mcp.server.tools.run_brain_extraction") as mock:
            ...
            mock.assert_called_once_with(input_node_id="v1", method="swiss", device="cpu")
    ...
    def test_export_model_with_format(self):
        ...
        with patch(target) as mock:
            ...
            mock.assert_called_once_with(
                node_id="m1", output_directory="/tmp", filename="mesh", file_format="OBJ"
            )
    ...
    def test_capture_3d_view_all_args(self):
        ...
        with patch(target) as mock:
            ...
            mock.assert_called_once_with(
                output_path="/tmp/v.png", width=800, height=600, view_index=1
            )
    ...
    def test_segment_vertebral_artery_all_args(self):
        ...
        with patch(target) as mock:
            ...
            mock.assert_called_once_with(
                input_node_id="v1", side="left", seed_points=[[1.0, 2.0, 3.0]]
            )
    ...
    def test_place_landmarks_with_labels(self):
        ...
        with patch(target) as mock:
            ...
            mock.assert_called_once_with(
                name="Pts", points=[[1, 2, 3], [4, 5, 6]], labels=["A", "B"]
            )
