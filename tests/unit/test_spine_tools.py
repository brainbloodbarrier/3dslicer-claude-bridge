...
class TestMeasureCCJAnglesValidation:
    ...
    def test_accepts_adult_population(self):
        ...
        with patch("slicer_mcp.features.spine.ccj.get_client") as mock_get_client:
            mock_client = Mock()
...
class TestMeasureSpineAlignmentValidation:
    ...
    def test_accepts_cervical_region(self):
        ...
        with patch("slicer_mcp.features.spine.alignment.get_client") as mock_get_client:
            mock_client = Mock()
            mock_client.exec_python.return_value = {
                "success": True,
                "result": json.dumps(
...
class TestSegmentSpineValidation:
    ...
    def test_valid_regions_accepted(self):
        ...
        for region in ["cervical", "thoracic", "lumbar", "full"]:
            ...
            with patch("slicer_mcp.features.spine.segmentation.get_client") as mock_get_client:
                mock_client = Mock()
                mock_client.exec_python.return_value = {
                    "success": True,
                    "result": (
...
class TestAnalyzeBoneQualityExecution:
    ...
    def test_successful_analysis(self):
        ...
        with patch("slicer_mcp.features.spine.analysis.get_client") as mock_get_client:
            mock_client = Mock()
            mock_client.exec_python.return_value = {
                "success": True,
...
class TestServerRegistration:
    ...
    def test_ccj_tool_handles_error(self):
        ...
        with patch("slicer_mcp.server.spine_tools.measure_ccj_angles") as mock_fn:
            mock_fn.side_effect = RuntimeError("test error")

            result = server_ccj(segmentation_node_id="vtkMRMLSegmentationNode1")
    ...
    def test_alignment_tool_handles_connection_error(self):
        ...
        with patch("slicer_mcp.server.spine_tools.measure_spine_alignment") as mock_fn:
            mock_fn.side_effect = SlicerConnectionError("Connection refused")

            result = server_align(segmentation_node_id="vtkMRMLSegmentationNode1")
